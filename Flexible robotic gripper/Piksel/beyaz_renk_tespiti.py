...
def getContours(img, cThr=[100, 100], showCanny=True, minArea=1000, filter=0, draw=False):
   ###### imgGray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ...
    lab = cv2.GaussianBlur(lab, (5, 5), 1)
    L, A, B = cv2.split(lab)
...
#cv2.imshow('gri',imgGray)
   ...
   # cv2.imshow('tonlu',imgGray)
    ...
    imgBlur = cv2.GaussianBlur(L_clahe, (5, 5), 1)
...
# C: küçük c  daha fazla beyaz çıkar, büyük c de ise daha fazla siyah çıkar.
    ...
    imgDial = cv2.dilate(imgThresh, kernel, iterations=3)
    imgThre = cv2.erode(imgDial, kernel, iterations=2)
    imgCanny = cv2.Canny(imgThre, cThr[0], cThr[1])

    cv2.imshow('kenar', imgCanny)
