...
cap = None
frame = None
gcode_command = None  # G-code komutu global olarak tutulur
        ...
        if len(conts) != 0:
            ...
            if len(conts2) != 0:
                for obj in conts2:
                    ...
                    # Sadece G-code hazırlanıyor
                    gcode_command = f"G1 X{101-(nW-4.5)*4.8} Y{(82-(nH-4.4)*4.945)}"
                    log_text.insert(tk.END, f"Hazır G-code: {gcode_command}\n")
...
def reset_command_flag():
    global gcode_command
    if not gcode_command:
        messagebox.showwarning("Uyarı", "Henüz bir ölçüme göre G-code oluşturulmadı.")
        return
    ...
    try:
        ...
        log_text.insert(tk.END, f"G-code gönderildi: {gcode_command}\n")
        gcode_command = None
    except Exception as e:
        messagebox.showerror("Hata", str(e))
...
# Arayüz
root = tk.Tk()
root.title("Görüntü ile Boyut Ölçümü ve G-code Gönderimi")
...
tk.Button(root, text="G28 Z0 Gönder", command=send_g28, width=25).pack(pady=10)
...
tk.Button(root, text="Yeni Ölçüm Gönder", command=reset_command_flag, width=25).pack(pady=10)
